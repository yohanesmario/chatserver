'use strict';

// Absolute Dependencies
var cc = require('app/tools/clusterCounter');
var numCPUs = cc.numCPUs;

// Environment variables
process.env.UV_THREADPOOL_SIZE = numCPUs; // libuv threadpool size

// Common Dependencies
var cluster = require('cluster');
var workerType = require('app/tools/workerType');

process.on('message', function(data){
    if (data!=null && data.messageType==="killWorker") {
        cluster.worker.kill();
    }
});

var startHTTP = function () {
    var http = require('http');
    var xml2js = require('xml2js');
    var xmlBuilder = new xml2js.Builder();
    var cfg = require('app/tools/readArgv').init(process);
    var guiGenerator = require('app/tools/guiGenerator');
    guiGenerator.init();
    var processPost = require('app/tools/processPost').init(cfg);

    // Create Server
    var server = http.createServer(function(request, response) {
        // var ip = request.headers['x-forwarded-for'] ||
        //          request.connection.remoteAddress ||
        //          request.socket.remoteAddress ||
        //          request.connection.socket.remoteAddress;
        // logWrapper.log(ip);
        switch (request.method) {
            case 'POST': {
                processPost.process(request, response);
            } break;
            case 'GET': {
                guiGenerator.handle(request, response);
            } break;
            default: {
                response.writeHead(200, "OK", {'Content-Type': 'text/plain'});
                response.end(xmlBuilder.buildObject({
                    response:{
                        $:{type:"c2s"},
                        "status":["rejected"],
                        "status-code":[12],
                        "message":["Unsupported method."]
                    }
                }));
            }
        }
    });

    // Establish listener.
    if (cfg.ip==null) {
        server.listen(cfg.port);
    } else {
        server.listen(cfg.port, cfg.ip);
    }

    process.send({
        messageType:"workerStarted"
    }, function(){
        process.send({
            messageType:"initWorker",
            payload:{
                workerID:cluster.worker.id
            }
        });
    });
};

var startTelnet = function() {
    require('app/c2s/telnetServer');

    process.send({
        messageType:"workerStarted"
    }, function(){
        process.send({
            messageType:"initWorker",
            payload:{
                workerID:cluster.worker.id
            }
        });
    });
};

var knowMyself = false;

process.on('message', function(data){
    if (knowMyself===false && data!=null && data.messageType==="youAre" && data.payload!=null && data.payload.workerType!=null) {
        switch(data.payload.workerType) {
            case workerType.TYPE_HTTP: {
                startHTTP();
                knowMyself = true;
            } break;
            case workerType.TYPE_TELNET: {
                startTelnet();
                knowMyself = true;
            } break;
        }
    }
});

process.send({
    "messageType":"whoAmI",
    "payload":{
        "workerID":cluster.worker.id+""
    }
});
