...
var _this = {
	...
	handle: function(request, response){
		...
		switch(method) {
            case 'registrasiServer': {
                _this.registrasiServer(request, response);
            } break;
            case 'loginServer': {
                _this.loginServer(request, response);
            } break;
            case 'registrasiClient': {
                _this.registrasiClient(request, response);
            } break;
            case 'loginClient': {
                _this.loginClient(request, response);
            } break;
            case 'chatSendClient': {
                _this.chatSendClient(request, response);
            } break;
            case 'logoutClient': {
                _this.logoutClient(request, response);
            } break;
            case 'logoutServer': {
                _this.logoutServer(request, response);
            } break;
            case 'serverHeartbeat': {
                _this.serverHeartbeat(request, response);
            } break;
            case 'listRegisteredServer': {
                _this.listRegisteredServer(request, response);
            } break;
            default: {
    ...
    registrasiServer:function(request, response){
        ...
        if (request!=null && request.param!=null && request.param.length===1 && request.param[0]!=null && request.param[0].$!=null && request.param[0].$.ip!=null && request.param[0].$.port!=null && request.param[0].$.password!=null) {
            ...
            _this.callbackHashTable['s2sWorker'+requestID] = function(data){
                var header = data.header;
                var res = data.data;
                response.writeHead(200, "OK", header);
                response.end(xmlBuilder.buildObject(res));

                delete _this.callbackHashTable['s2sWorker'+requestID];
            };
            process.send({
        ...
        process.on('message', function(data){
            if (data.messageType!=null) {
                switch (data.messageType) {
                    case "s2sWorker": {
                        _this.callbackHashTable["s2sWorker"+data.payload.reqID](data.payload);
                    } break;
                }
            }
        });
