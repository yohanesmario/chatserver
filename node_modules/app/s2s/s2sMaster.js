...
var _this = {
        ...
        if (res!=null && res.length===1 && res[0].ipPort===ipPort) {
            if (res[0].password===hash) {
                if (res[0].loggedIn!==true) {
                    ...
                    var usrHistoryRes = _this.dbWrapper.userHistory.find({'$loki':{'$gt':-Infinity}});
                    var messages = [];
                    var userHistory = [];
                    var usersBucket = [];
                    var i;
                    ...
                    for (i = 0; i < usrHistoryRes.length; i++) {
                        userHistory.push({
                            "$":{
                                "username":usrHistoryRes[i].username,
                                "timestamp":usrHistoryRes[i].timestamp,
                                "action":usrHistoryRes[i].action
                            }
                        });
                    }
    ...
    registrasiClient:function(request){
        ...
        var workerID = request.workerID;
        request = request.data;
        var lastAction = "register";
        var lastTimestamp = [parseInt(request.param[0].$.timestamp.split("::")[0]), request.param[0].$.timestamp.split("::")[1]];

        var srv = _this.dbWrapper.servers.find({"sessid":request.param[0].$.sessid});
        if (srv!=null && srv.length===1) {
            let probe = _this.dbWrapper.users.find({"username":request.param[0].$.username});
            let res = null;
            if (probe.length>=1) {
                if (probe[0].lastAction===lastAction && (probe[0].lastTimestamp[0]>lastTimestamp[0] || (probe[0].lastTimestamp[0]===lastTimestamp[0] && probe[0].lastTimestamp[1]>lastTimestamp[1]))) {
                    // OVERRIDE
                    probe[0].password = request.param[0].$.password;
                    probe[0].loggedIn = false;
                    probe[0].sessid = null;
                    probe[0].heartbeat = null;
                    probe[0].handledByMe = false;
                    probe[0].handlerIpPort = null;
                    probe[0].lastTimestamp = lastTimestamp;
                    res = probe[0];
                    _this.dbWrapper.users.update(probe[0]);
                    _this.dbWrapper.queueSaving();
                }
            } else {
                // NOTHING TO OVERRIDE
                res = _this.dbWrapper.users.insert({
    ...
    loginClient:function(request){
        ...
        var lastTimestamp = [parseInt(request.param[0].$.actiontimestamp.split("::")[0]), request.param[0].$.actiontimestamp.split("::")[1]];

        var srv = _this.dbWrapper.servers.find({"sessid":request.param[0].$.sessid});
        if (srv!=null && srv.length===1) {
            var res = _this.dbWrapper.users.find({"username":request.param[0].$.username});
            if (res!=null && res.length===1) {
                if (res[0].lastAction!==lastAction || (res[0].lastAction===lastAction && (res[0].lastTimestamp[0]>lastTimestamp[0] || (res[0].lastTimestamp[0]===lastTimestamp[0] && res[0].lastTimestamp[1]>lastTimestamp[1])))) {
                    res[0].loggedIn = true;
                    res[0].sessid = request.param[0].$.csessid;
                    res[0].heartbeat = null;
                    res[0].handledByMe = false;
                    res[0].handlerIpPort = srv[0].ipPort;
                    res[0].lastAction = lastAction;
                    res[0].lastTimestamp = lastTimestamp;
                    ...
                    if (request.param[0].$.timestamp!=null) {
                        let ts = parseInt(request.param[0].$.timestamp);
    ...
    logoutClient:function(request){
        ...
        if (srv!=null && srv.length===1) {
            ...
            if (res!=null && res.length===1) {
                ...
                res[0].lastAction = "logout";
                res[0].lastTimestamp = Date.now();
                _this.dbWrapper.users.update(res[0]);
