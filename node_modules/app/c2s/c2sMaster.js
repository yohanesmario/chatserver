...
var _this = {
    ...
    initWorker:function(workerID){
        var res = _this.dbWrapper.chatHistory.chain().find({}).simplesort('$loki').data();
        var msgRes = [];
        ...
        if (res!=null && res.password!=null) {
            if (res.loggedIn===false) {
                if (res.password===hash) {
                    ...
                    _this.s2sClient.loginClient(username, sessid, function(){});
                } else {
                    // Fail
                    cluster.workers[workerID].send({
                        messageType:"c2sWorker",
    ...
    logout: function(request){
        ...
        if (res!=null && res.length>0 && res[0].sessid===sessid) {
            ...
            _this.s2sClient.logoutClient(sessid, function(){});
        } else {
            cluster.workers[workerID].send({
                messageType:"c2sWorker",
                payload:{
