...
var _this = {
    ...
    login: function(request){
        ...
        var lastTimestamp = [Date.now(), _this.cfg.identifier];
        ...
        if (res!=null && res.password!=null) {
            if (res.loggedIn===false) {
                if (res.password===hash) {
                    ...
                    res.lastTimestamp = lastTimestamp;
                    ...
                    let ts = Date.now();
                    _this.s2sClient.loginClient(username, sessid, ts, lastTimestamp, function(){});
                    _this.dbWrapper.userHistory.insert({
                        "username":username,
                        "timestamp":ts,
                        "actiontimestamp":lastTimestamp,
                        "action":"LOGIN"
                    });
                    _this.dbWrapper.queueSaving();
    ...
    register: function(request){
        ...
        if (res.username===username) {
            ...
            _this.s2sClient.registerClient(username, password, lastTimestamp, function(){});
        } else {
            // Fail
            cluster.workers[workerID].send({
                messageType:"c2sWorker",
