...
module.exports.logoutAllUsers = function(cb){
    ...
    async.each(users, function(user, callback){
        if (user.loggedIn===true) {
            module.exports.userHistory.insert({
...
module.exports.loadDatabase = function(callback){
    module.exports.db.loadDatabase({}, function(){
        ...
        module.exports.userHistory = module.exports.db.getCollection('userHistory');
        ...
        if (module.exports.userHistory==null) {
            module.exports.userHistory = module.exports.db.addCollection('userHistory');
        }
        ...
        function cleanupUser(usersObj) {
            var invalidUsers = usersObj.find({'$and':[{'loggedIn':true}, {'handledByMe':true}, {'heartbeat':{'$lt':(Date.now()-(10*1000))}}]});

            for (var i = 0; i < invalidUsers.length; i++) {
                process.emit('logoutUser', {
                    "username":invalidUsers[i].username,
                    "timestamp":Date.now(),
                    "sessid":invalidUsers[i].sessid
                });
        ...
        function cleanupServer(serversObj, usersObj) {
            var invalidServers = serversObj.find({'$and':[{'loggedIn':true}, {'heartbeat':{'$lt':(Date.now()-(10*1000))}}]});
            var invalidUsers;

            for (var i = 0; i < invalidServers.length; i++) {
                process.emit('logoutServer', {
                    "sessid":invalidServers[i].sessid
                });
                ...
                for (var j = 0; j < invalidUsers.length; j++) {
                    process.emit('logoutUser', {
                        "username":invalidUsers[j].username,
                        "timestamp":Date.now(),
                        "sessid":invalidUsers[j].sessid
                    });
