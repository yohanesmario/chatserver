...
module.exports.init = function(processObj) {
    ...
    if (processObj.argv.length>=3) {
    	...
    	for (i=2; i<processObj.argv.length; i++) {
            if (processObj.argv[i]!=="-w") {
                argvBucket += processObj.argv[i];
                if (i+1<processObj.argv.length && processObj.argv[i+1]!=="-w") {
                    argvBucket += ' ';
                }
            } else {
                i = processObj.argv.length;
            }
    	}
        ...
        if (config!=null) {
            if (config.port!=null) {
                let tempPort = config.port+"";
                if (parseInt(tempPort.charAt(0))===1) {
                    tempPort = tempPort.substring(1,tempPort.length);
                }
                port = parseInt("1"+tempPort);
                identifier.port = port;
                telnetPort = parseInt("2"+tempPort);
            }
            if (config.ip!=null) {
                ip = config.ip;
                identifier.ip = ip;
